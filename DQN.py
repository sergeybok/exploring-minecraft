...
class Qnetwork():
    def __init__(self):
        ...
        #TODO define the frame size here
        flattened_frame_size = None
        frame_width = None
        frame_height = None
        frame_channels = 3
        ...
        self.conv1 = self.conv_layer(input_volume=self.reshaped_image, num_output_channel=32, filter_shape=[8, 8], strides_shape=[1, 4, 4, 1], padding_type='VALID', layer_name='1')
        self.conv2 = self.conv_layer(input_volume=self.conv1, num_output_channel=64, filter_shape=[4, 4], strides_shape=[1, 2, 2, 1], padding_type='VALID', layer_name='2')
        self.conv3 = self.conv_layer(input_volume=self.conv2, num_output_channel=128, filter_shape=[3, 3], strides_shape=[1, 1, 1, 1], padding_type='VALID', layer_name='3')
        self.conv4 = self.conv_layer(input_volume=self.conv3, num_output_channel=512, filter_shape=[7, 7], strides_shape=[1, 1, 1, 1], padding_type='VALID', layer_name='4')
    ...
    def conv_layer(self, input_volume = None, num_output_channel = None, filter_shape = None, strides_shape = None, padding_type = None, layer_name = None,):
        std_dev = 0.1
        W_shape = [filter_shape[0], filter_shape[1], input_volume.shape[3].value, num_output_channel]
        W = tf.Variable(tf.truncated_normal(W_shape, stddev=std_dev), name='Filter'+layer_name)
        b_shape = [num_output_channel]
        b = tf.Variable(tf.constant(0.1, shape=b_shape), name='bias'+layer_name)
        conv = tf.nn.conv2d(input_volume, W, strides_shape, padding='SAME', name='Conv'+layer_name)
        conv = conv + b
        relu = tf.nn.relu(conv, name='ReLU'+layer_name)
        return relu
        # pool1_ksize = [1, 2, 2, 1]
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < max_epLength:  # If the agent takes longer than 200 moves to reach either of the blocks, end the trial.
            ...
            # s1, r, is_terminal_flag = env.step(a)
            ...
            if total_steps > pre_train_steps:
                ...
                if total_steps % (update_freq) == 0:
                    ...
                    #TODO figure out the use of end_multiplier???
